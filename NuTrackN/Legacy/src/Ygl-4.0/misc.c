...
static void ortho2_x11(const char * caller,
		       Coord left, Coord right, Coord bottom, Coord top) {
  YglWindow *w = W;
  if(left == right || bottom == top) {
    Yprintf(caller, "x-range or y-range is empty.\n");
    return;
  }
  w->ol = left;
  w->or = right;
...
void ortho2(Coord left, Coord right, Coord bottom, Coord top) {
  ...
  IFOGL(ortho(left, right, bottom, top, -1.0, 1.0),
	ortho2_x11(MyName, left, right, bottom, top));
}
...
void viewport(Screencoord left,   Screencoord right,
  ...
  I(MyName);
  IFOGL(glViewport(left, bottom, right - left, top - bottom),
	viewport_x11(left, right, bottom, top));
}
...
Int32 windepth(Int32 wid) {
  Window root, parent, *children, main;
  Uint nchildren;
  Int32 n = 0;
...
# if defined(__i386) ||defined(__x86_64__) || defined(__alpha)
