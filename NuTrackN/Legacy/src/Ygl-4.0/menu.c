...
static int 	white, black,yellow,blue;
...
static void create_menu(YglMenu *m) {
  if(menugc == NULL) {
    ...
    yellow = YGL_COLORS(YELLOW);
    blue = YGL_COLORS(BLUE);
  }
  {
    Ulong mask = 0;
    ...
    m->win = XCreateWindow(D, getXwid(),  /*/RootWindow(D, YglScreen),*/
			   0, 0,
			   m->w + MENUSHADE,
			   m->h + MENUSHADE,
			   1,
...
#endif
  xx = MIN(m->x - m->w/2 - MENUSHADE, xx - m->w + MENUSHADE - 5 );
  if( xx < 5 ) xx = 5;
  
  yy = MIN(m->y, yy - m->h + MENUSHADE - 5 );
  if( yy < 5 ) yy = 5;
