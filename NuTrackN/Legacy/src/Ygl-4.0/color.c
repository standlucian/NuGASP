...
void color(Colorindex ind) {
  ...
  if(W->rgb)
    ...
    W->color = ind;
    IFOGL(glIndexs(YGL_COLORS(ind)),
	  if(Ygl.GC) XSetForeground(D, W->gc, YGL_COLORS(ind)); /* single GC */
	  else       W->gc = W->gclist[YGL_COLORS(ind)]
	  );
  }
}
...
#undef ZOOMDATA
    ...
    if(W->rgb) {
      Ulong pixel = RGB24_TO_RGBVisual(val);
