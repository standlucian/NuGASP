...
/* Points */
void pnt2  (Coo x, Coo y       ) { I("pnt2" ); PNT2_FN(f, x, y); F; }
void pnt2i (Ico x, Ico y       ) { I("pnt2i"); PNT2_FN(i, x, y); F; }
void pnt2s (Sco x, Sco y       ) { I("pnt2s"); PNT2_FN(s, x, y); F; }
void pnt   (Coo x, Coo y, Coo z) { I("pnt"  ); PNT_FN(f, x, y, z); F; }
void pnti  (Ico x, Ico y, Ico z) { I("pnti" ); PNT_FN(i, x, y, z); F; }
void pnts  (Sco x, Sco y, Sco z) { I("pnts" ); PNT_FN(s, x, y, z); F; }
...
void draw2 (Coo x, Coo y       ) { I("draw2" ); DRAW2_FN(f, x, y); SCP( =x, =y, =0); F;}
void draw2i(Ico x, Ico y       ) { I("draw2i"); DRAW2_FN(i, x, y); SCP( =x, =y, =0); F;}
void draw2s(Sco x, Sco y       ) { I("draw2s"); DRAW2_FN(s, x, y); SCP( =x, =y, =0); F;}
void draw  (Coo x, Coo y, Coo z) { I("draw"  );  DRAW_FN(f, x, y, z); SCP( =x, =y, =z); F;}
void drawi (Ico x, Ico y, Ico z) { I("drawi" );  DRAW_FN(i, x, y, z); SCP( =x, =y, =z); F;}
void draws (Sco x, Sco y, Sco z) { I("draws" );  DRAW_FN(s, x, y, z); SCP( =x, =y, =z); F;}

void rdr2  (Coo x, Coo y       ) { I("rdr2"  ); DRAW2_FN(f, W->xp+x, W->yp+y); SCP(+=x,+=y,+=0); F;}
void rdr2i (Ico x, Ico y       ) { I("rdr2i" ); DRAW2_FN(i, W->xp+x, W->yp+y); SCP(+=x,+=y,+=0); F;}
void rdr2s (Sco x, Sco y       ) { I("rdr2s" ); DRAW2_FN(s, W->xp+x, W->yp+y); SCP(+=x,+=y,+=0); F;}
void rdr   (Coo x, Coo y, Coo z) { I("rdr"   );  DRAW_FN(f, W->xp+x, W->yp+y, W->zp+z); SCP(+=x,+=y,+=z); F;}
void rdri  (Ico x, Ico y, Ico z) { I("rdri"  );  DRAW_FN(i, W->xp+x, W->yp+y, W->zp+z); SCP(+=x,+=y,+=z); F;}
void rdrs  (Sco x, Sco y, Sco z) { I("rdrs"  );  DRAW_FN(s, W->xp+x, W->yp+y, W->zp+z); SCP(+=x,+=y,+=z); F;}
...
#endif
void arc   (Coo x, Coo y, Coo r, Angle s, Angle e) { I("arc"   );  ARC_FN(x, y, r, s, e); F;}
void arci  (Ico x, Ico y, Ico r, Angle s, Angle e) { I("arci"  );  ARC_FN(x, y, r, s, e); F;}
void arcs  (Sco x, Sco y, Sco r, Angle s, Angle e) { I("arcs"  );  ARC_FN(x, y, r, s, e); F;}
void arcf  (Coo x, Coo y, Coo r, Angle s, Angle e) { I("arcf"  ); ARCF_FN(x, y, r, s, e); F;}
void arcfi (Ico x, Ico y, Ico r, Angle s, Angle e) { I("arcfi" ); ARCF_FN(x, y, r, s, e); F;}
void arcfs (Sco x, Sco y, Sco r, Angle s, Angle e) { I("arcfs" ); ARCF_FN(x, y, r, s, e); F;}

void circ  (Coo x, Coo y, Coo r) { I("circ"  );  ARC_FN(x, y, r, 0, 3600); F;}
void circi (Ico x, Ico y, Ico r) { I("circi" );  ARC_FN(x, y, r, 0, 3600); F;}
void circs (Sco x, Sco y, Sco r) { I("circs" );  ARC_FN(x, y, r, 0, 3600); F;}
void circf (Coo x, Coo y, Coo r) { I("circf" ); ARCF_FN(x, y, r, 0, 3600); F;}
void circfi(Ico x, Ico y, Ico r) { I("circfi"); ARCF_FN(x, y, r, 0, 3600); F;}
void circfs(Sco x, Sco y, Sco r) { I("circfs"); ARCF_FN(x, y, r, 0, 3600); F;}

/* Rects & Boxes */
void rect  (Coo x1, Coo y1, Coo x2, Coo y2) { I("rect"  );  RECT_FN(f, x1, y1, x2, y2); F;}
void sbox  (Coo x1, Coo y1, Coo x2, Coo y2) { I("sbox"  );  RECT_FN(f, x1, y1, x2, y2); F;}
void recti (Ico x1, Ico y1, Ico x2, Ico y2) { I("recti" );  RECT_FN(i, x1, y1, x2, y2); F;}
void sboxi (Ico x1, Ico y1, Ico x2, Ico y2) { I("sboxi" );  RECT_FN(i, x1, y1, x2, y2); F;}
void rects (Sco x1, Sco y1, Sco x2, Sco y2) { I("rects" );  RECT_FN(s, x1, y1, x2, y2); F;}
void sboxs (Sco x1, Sco y1, Sco x2, Sco y2) { I("sboxs" );  RECT_FN(s, x1, y1, x2, y2); F;}
void rectf (Coo x1, Coo y1, Coo x2, Coo y2) { I("rectf" ); RECTF_FN(f, x1, y1, x2, y2); F;}
void sboxf (Coo x1, Coo y1, Coo x2, Coo y2) { I("sboxf" ); RECTF_FN(f, x1, y1, x2, y2); F;}
void rectfi(Ico x1, Ico y1, Ico x2, Ico y2) { I("rectfi"); RECTF_FN(i, x1, y1, x2, y2); F;}
void sboxfi(Ico x1, Ico y1, Ico x2, Ico y2) { I("sboxfi"); RECTF_FN(i, x1, y1, x2, y2); F;}
void rectfs(Sco x1, Sco y1, Sco x2, Sco y2) { I("rectfs"); RECTF_FN(s, x1, y1, x2, y2); F;}
void sboxfs(Sco x1, Sco y1, Sco x2, Sco y2) { I("sboxfs"); RECTF_FN(s, x1, y1, x2, y2); F;}
...
void poly2 (Int32 n, Coo p[][2]) { const char *MyName = "poly2" ; I(MyName); IFOGL(NI(MyName),MKP(n, p); q_add(MyName, XS(p[0][0]), YS(p[0][1])); XDrawLines(DWGC, queue, q_len, CoordModeOrigin)); F;}
void poly2i(Int32 n, Ico p[][2]) { const char *MyName = "poly2i"; I(MyName); IFOGL(NI(MyName),MKP(n, p); q_add(MyName, XS(p[0][0]), YS(p[0][1])); XDrawLines(DWGC, queue, q_len, CoordModeOrigin)); F;}
void poly2s(Int32 n, Sco p[][2]) { const char *MyName = "poly2s"; I(MyName); IFOGL(NI(MyName),MKP(n, p); q_add(MyName, XS(p[0][0]), YS(p[0][1])); XDrawLines(DWGC, queue, q_len, CoordModeOrigin)); F;}

void polf2 (Int32 n, Coo p[][2]) { const char *MyName = "polf2" ; I(MyName); IFOGL(NI(MyName),MKP(n, p); SCP(=p[0][0],=p[0][1], =0); XFillPolygon(DWGC, queue, q_len, PMode, CoordModeOrigin)); F;}
void polf2i(Int32 n, Ico p[][2]) { const char *MyName = "polf2i"; I(MyName); IFOGL(NI(MyName),MKP(n, p); SCP(=p[0][0],=p[0][1], =0); XFillPolygon(DWGC, queue, q_len, PMode, CoordModeOrigin)); F;}
void polf2s(Int32 n, Sco p[][2]) { const char *MyName = "polf2s"; I(MyName); IFOGL(NI(MyName),MKP(n, p); SCP(=p[0][0],=p[0][1], =0); XFillPolygon(DWGC, queue, q_len, PMode, CoordModeOrigin)); F;}

#ifdef OGL
static void polf_ogl(Int32 n, Coo p[][3]) {
  int i;
  glBegin(GL_POLYGON);
  for(i = 0; i < n; i++) glVertex3fv(p[i]);
  glEnd();
}
#endif

void polf  (Int32 n, Coo p[][3]) { const char *MyName = "polf" ; I(MyName); IFOGL(polf_ogl(n, p),MKP(n, p); SCP(=p[0][0],=p[0][1], =0); XFillPolygon(DWGC, queue, q_len, PMode, CoordModeOrigin)); F;}
...
void endtmesh     (void) { const char *MyName = "endtmesh"     ; if(W->vmode != VertexTMesh) {Yprintf(MyName, "missing bgntmesh.\n"     ); exit(1);} IFOGL(glEnd(), NI(MyName)); F;}
...
#ifdef OGL
void cmov  (Coo x, Coo y, Coo z) { W->xc = x; W->yc = y; W->zc = z;}
void cmovi (Ico x, Ico y, Ico z) { W->xc = x; W->yc = y; W->zc = z;}
void cmovs (Sco x, Sco y, Sco z) { W->xc = x; W->yc = y; W->zc = z;}
#endif
