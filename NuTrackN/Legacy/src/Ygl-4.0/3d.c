/*
 *    Ygl: Run GL programs with OpenGL routines.
 *    (C) Fred Hucht 1996
 *    EMail: fred@thp.Uni-Duisburg.DE
 */

static char vcid[] = "$Id: 3d.c,v 4.6 1998-07-26 13:02:07+02 fred Exp $";
...
void mmode(Int16 mode) {
  ...
  switch(mode) {
  case MSINGLE:
    Yprintf(MyName, "MSINGLE not implemented...\n");
    break;
  case MPROJECTION: glMatrixMode(GL_PROJECTION); break;
  case MVIEWING:    glMatrixMode(GL_MODELVIEW);  break;
  default:
...
static void deflightmodel(const char *caller, 
  ...
  case LOCALVIEWER:
    ...
    break;
  case ATTENUATION:
    lightmodel->attenuation;
    /* Use glLight... */
    Yprintf(caller, "LMODEL ATTENUATION not implemented.\n");
    break;
  case TWOSIDE:
...
void lmbind(Int32 target, Int32 index) {
  ...
  if(!Ygl.UseOGL) NI(MyName);
#ifdef DEBUG
  Yprintf(MyName, "target=%d, index=%d\n", target, index);
#endif
