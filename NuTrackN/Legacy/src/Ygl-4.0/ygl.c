...
YglControl Ygl = {
  ...
  True ,		/* BS */	/* YGL_BS (backing store ) set */
  False,		/* GC */	/* Use single GC in CMap windows (slower) */
  True,		/* cmapmode uses TrueColor or DirectColor visual */
#ifdef OGL
...
static void create_main_window(const char *caller, const char *Title,
  ...
  swa.background_pixel = BlackPixel(D,YglScreen);
...
static void setup_shift_and_mask() {
...
static int setup_visuals_ogl(const char *caller) { /* find Visuals */
  ...
  if(NULL == (xv = glXChooseVisual(D, YglScreen, cmaplist))) {
    Yprintf(caller, "no OpenGL colormap visual found.\n");
    return False;
  }
  ...
  if(NULL == (xv = glXChooseVisual(D, YglScreen, rgblist))) {
...
#ifdef HASXMU
static void find_colormap_xmu(const char *caller) { /* N.M. Original void*/
...
#ifdef DEBUG
	fprintf(stderr, "alloc color %s, i = %d, pixel = %d (%d,%d,%d)\n",
...
void ginit(void) {
  ...
  Ygl.BS = ( DoesBackingStore( DefaultScreenOfDisplay(D) ) == Always) &&
